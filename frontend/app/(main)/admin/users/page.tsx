...
export default function UsersPage() {
  ...
  const formatPlatform = (platform?: string) => {
    const platformMap: { [key: string]: { name: string; color: string } } = {
      ...
      community: { name: '敲鸭', color: 'bg-accent text-primary' },
      google: { name: 'Google', color: 'bg-background text-destructive' },
      wechat: { name: '微信', color: 'bg-success-subtle text-success' }
    };
  ...
  return (
    <div className="space-y-6">
      {/* 页面标题 */}
      <div>
        <h1 className="text-2xl font-bold text-foreground">{t("用户管理")}</h1>
        <p className="text-muted-foreground mt-1">{t("查看系统中的所有用户账户信息")}</p>
      </div>
                  ...
                  <TableCell>
                    {user.githubLogin ? (
                      <div className="text-sm">
                        ...
                        <div className="text-muted-foreground">{t("ID")}: {user.githubId}</div>
                      </div>
                    ) : (
                      <div className="text-sm text-muted-foreground">{t("未绑定")}</div>
                    )}
                  </TableCell>
                  <TableCell>
