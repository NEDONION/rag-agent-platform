...
>
> **CI 部署无需手工准备这个变量**：`deploy.yml` 的「确保必填密钥存在」步骤会在
> 服务器 `.env` 中缺失时生成一次并写入，之后每次部署检测到已存在即跳过。
> 已验证幂等：连续多次部署密钥不变、不产生重复行、能修复「变量存在但值为空」。
>
> **绝不能改成每次部署重新生成**——换了密钥，该环境已存的服务商配置就再也解不开，
> 用户必须重新填写全部 API Key。手动部署时同理，首次生成后请妥善保存。
